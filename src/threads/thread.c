...
static bool priority_cmp_func(const struct list_elem *a,
...
thread_yield (void) 
{
  ...
  ASSERT (!intr_context ());
  printf("thread %02d-%s yield on tick %lld.\n", cur->tid, cur->name, timer_ticks());
...
/* azazo1: Let the thread be in the sleep state until the timer interrupt wakes it up. */
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  old_level = intr_disable ();
  list_insert_ordered (&all_list, &t->allelem, priority_cmp_func, NULL);
  intr_set_level (old_level);
}
...
bool
priority_cmp_func(const struct list_elem *a,
                  const struct list_elem *b,
                  void *aux UNUSED) {
